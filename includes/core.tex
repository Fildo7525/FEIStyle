...
sú pripojené na meniče. Tie sú ovládané priamo príkazmi z počítača. Príkazy sa na počítač posielajú cez sieť z externého počítača. Príkazy sa posielajú
na počítač vo formáte \textbf{JSON}. Študenti, ktorí navrhovali systém posielania požiadavok (request) a odpovedí (response) robili tieto správu ručne. Preto nastávajú situácie,
kedy robot posie správu, ktorá nespadá do štandardu písania JSON textu. Z tohto dôvodu sme nemohli použiť už existujúci kód, ktorý by nám zjednodušil prehľadávanie týchto sprav.

\noindent Podla dokumentácie sa robot mal ovládať správami typu~\cite{BMdoc}
...
\begin{itemize}
	\item \textbf{UserID} Znázorňuje ID užívateľa, ktorý je pripojený na robot. Predvolená hodnota je 1. Tato možností je v momentálnom stave robota nevyužitá.
		Počet zariadení, ktoré sa kozu pripojiť na port, cez ktorý sa dá robot ovládať je 1.
	\item \textbf{Command} Číselná hodnota znázorňujúca príkaz, ktorý ma robot vykonať
		\begin{enumerate}
			\setcounter{enumi}{-1}
			\item Prázdny príkaz skučiaci na overenie spojenia
			\item Núdzové zastavenie
			\item Normálne zastavenie
			\item Príkaz nastavujúc rýchlosti kolies mobilného robota
			\item Prázdny príkaz % My sme ho neskor prepísali na príkaz, cez ktorý sa dá nastaviť žiadaná pozícia kolies robota (natočenie).
			\item Prázdny príkaz
			\item Príkaz pýtajúc si aktuálnu rýchlosti pravého a ľavého kolesa. Tento príkaz nebol sprave navrhnutý v kóde robota. Vracal nám žiadaná hodnotu
				namiesto aktuálnej. Museli sme ho prepísať
			\item Pripravenie motorov robota
			\item Prázdny príkaz % Príkaz pýtajúci si aktuálnu pozíciu pravého a ľavého kolesa.
		\end{enumerate}
...
Toto bolo písané v dokumentácii, ktorá nám bola dodaná na začiatku programu. Mozeme preto priklad prepisat na retazec, ktory by fungoval
